...
Everything you typically want to do in statistical programming uses **functions**. `mean` is a good example. `mean` takes one **argument**, a numeric vector. Pipes are a way to write strings of functions more easily. They bring the first argument of the function to the bedginning. So instead of writing `mean(iris$Petal.length)` you can write:
...
But this output just feels overwhelming and uninformative. 
